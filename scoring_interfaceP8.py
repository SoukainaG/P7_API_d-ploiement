...
# Sélection du client via SK_ID_CURR
client_id = st.selectbox("Sélectionnez un client (SK_ID_CURR)", options=clients_df['SK_ID_CURR'])

# Récupération de l'index correspondant dans le DataFrame
client_index = clients_df.index[clients_df['SK_ID_CURR'] == client_id].tolist()[0]

# Extraction des caractéristiques du client sélectionné
features = important_features.iloc[client_index].tolist()
